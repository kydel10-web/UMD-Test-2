...
const PROJECTS = [
  {
    ...
    images: [
      ...
      "/images/jackson-transverse-polish.png",
          ...
          {artworks.map(({ src, title, medium }) => (
            <div key={src} className="group cursor-zoom-in bg-neutral-100 rounded-lg overflow-hidden" onClick={() => setSelected({ src, title })}>
              ...
              <div className="mt-3 px-1 pb-3 flex justify-between items-baseline">
                <h3 className="text-base font-serif">{title}</h3>
                <p className="text-base font-serif text-muted">{medium}</p>
              </div>
            </div>
          ))}
        </div>
...
function LandingPage() {
  ...
  useEffect(() => {
    const handleScroll = () => {
      ...
      let overExpertise = false;
      if (expertiseRef.current) {
        const rect = expertiseRef.current.getBoundingClientRect();
        overExpertise = rect.top < 100 && rect.bottom > 0;
      }
  ...
  return (
    <div className="min-h-screen selection:bg-ink selection:text-bg">
      {/* Navigation */}
      <nav className={`fixed top-0 left-0 w-full z-50 px-6 py-8 flex justify-between items-center transition-colors duration-300 ${navOnDark ? "text-white" : "text-ink"}`}>
        <motion.div
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-16 items-start">
            ...
            >
              ...
              <div className="mt-8">
                ...
                <p className="text-xs uppercase tracking-[0.3em] text-muted mt-2">Assoc. AIA</p>
              </div>
            </motion.div>
            ...
            >
              <div className="space-y-6 text-muted leading-relaxed text-lg">
                <p className="italic border-l-2 border-accent pl-4">
                ...
                <p>
                  I value intentional, innovative, and high-performance design. My approach blends conceptual exploration with digital fabrication to translate ideas into building systems that are both expressive and pragmatic. I intend on becoming a Licensed Architect in Maryland, Virginia, and Washington D.C. and obtain my LEED Green Associate and LEED BD+C certifications to design buildings that are efficient and environmentally impactful.
                </p>
              </div>
      ...
      {/* Services / Approach */}
      <section ref={expertiseRef} className="py-32 px-6 md:px-24 bg-navy text-bg overflow-hidden relative">
        <div className="max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-12 gap-12">
          ...
          <div className="md:col-span-8 space-y-24">
            {[
              { title: "Object Design", desc: "The dialogue between architecture and object through form, material, and concept." },
              { title: "Parametric Design", desc: "Using Algorithm thinking to define form and structure from facade to detail." }
            ].map((service, i) => (
              <motion.div 
                key={i}
